...
fn test_initialize() {
    ...
    assert_eq!(res.attributes.get(4).unwrap().value, "0".to_string());
}
...
fn test_eat() {
    ...
    assert_eq!(0, res.messages.len());
    assert_eq!(res.attributes.get(0).unwrap().value, "try_eat".to_string());
    assert_eq!(res.attributes.get(1).unwrap().value, "1".to_string());
}

#[test]
fn test_eat_unauthorized() {
    let CommonTest {
        mut deps,
        mut info,
        res: _,
...
fn test_eat_locked() {
    ...
    // attempt to eat a locked ownable
    let err = execute(
        deps.as_mut(),
        create_lto_env(),
        mock_info(LTO_USER, &[]),
...
fn test_eat_too_much() {
    ...
    let _expected_err_val = "attempt to eat more than possible".to_string();
...
fn test_query_config() {
    ...
    let json: String = "{\"current_amount\":3,\"max_capacity\":3,\"}".to_string();
...
fn test_query_metadata() {
    ...
    let resp: Binary = query(deps.as_ref(), create_lto_env(), msg).unwrap();
    let json: String = "{\"image\":null,\"image_data\":null,\"external_url\":null,\"description\":\"Have a bite\",\"name\":\"Cookie\",\"background_color\":null,\"animation_url\":null,\"youtube_url\":null}".to_string();
    let expected_binary = Binary::from(json.as_bytes());
